...
@EXPORT_OK = qw(
  ...
  set_timestamping     get_timestamping
  ...
  open_log close_log is_logging get_log_fh
...
# Are we timestamping output?
my $timestamp_output = 0;
sub set_timestamping { $timestamp_output = $_[0] || 0 }
sub get_timestamping { return $timestamp_output }
...
=cut
    ...
    sub open_log {
        ...
        if ( !defined $logfile ) {
            ...
            $dir =~ s/\..*$//;
            if ( -d ".logs/$dir" ) {
                $logfile = ".logs/$dir/$dir.";
            } else {
                $logfile = ".logs/common/$dir.";
            }
            ...
            $logfile .= join '.' => $user, hostname(), time, $$;
            $defaultlogfile = $logfile;

            # A log in the default location, which means we need to
            # make sure that it's group readable
            $should_chmod = 1;
        }
        ...
        # Should we chmod it? If so make the file group-readable
        if ($should_chmod) {
            my $perm = ( stat $logh )[2] & 07777;
            chmod( $perm | 060, $logh );
        }
        $logf = $logfile;
...
=cut
    ...
    sub log_proctree {
        ...
        if ( $^O eq 'linux' ) {
            my @pstree_output;
            my $current_pid = $$;
            while ($current_pid) {    # stop at pid 0
                open my $psinfo, '<', "/proc/$current_pid/status"
                  or last;
                my %attrs = map { /^\s*(\S+)\:\s*(.+)\n$/ } <$psinfo>;
                last if $attrs{Uid} !~ /\b$<\b/;
                my $cmdline = '';
                if ( open my $cmdline_fh, '<', "/proc/$current_pid/cmdline" ) {
                    local $/ = undef;
                    $cmdline = <$cmdline_fh>;
                    $cmdline =~ s/\0/ /g;
                    $cmdline =~ s/^\s*|\s*$//g;
                }
                if ( $cmdline eq $attrs{Name} ) {
                    push @pstree_output, $attrs{Name};
                } else {
                    push @pstree_output, "$attrs{Name} ($cmdline)";
                }
                $current_pid = $attrs{PPid};
            }
            verbose( "Proc tree:\n" . join '---', reverse @pstree_output );
        } else {
            my $proc =
...
=cut
    ...
    sub m_message ($@) {
        ...
        for my $mess ( $msg, $tmsg, $rmsg ) {
            $mess = "$mess @_" if @_;
            $mess = "$mess\n" unless "\n" eq substr $mess, -1;
        }

        # unless quiet, print to STDERR
        print STDERR ( $timestamp_output ? $tmsg : $msg ) unless $quiet;
    ...
    sub _a_message {
        ...
        my $localprog      = $prog;
        $prog = '';
        my $localtimestampoutput = $timestamp_output;
        $timestamp_output = '';
        my $localprefix = $prefix;
        $prefix = '';
        ...
        $prog             = $localprog;
        $timestamp_output = $localtimestampoutput;
        $prefix           = $localprefix;
        return 1;
