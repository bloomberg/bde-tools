...
sub getoptions {
    ...
    if ($opts{caller} && ($opts{text} || $opts{data} || $opts{bss})) {
...
MAIN: {
  ...
  foreach my $sym (sort @symparams) {
	...
	$binobj = Binary::Object->new($sym);
      ...
      my @binsyms;
      if ($opts->{caller} || $opts->{lookup}) {
	@binsyms = $binobj->getDefinedSymbols();
	@binsyms = grep {my $type = $_->getType;
			 ($type eq 'D' && $opts->{data}) ||
			 ($type eq 'T' && $opts->{text}) ||
			 ($type eq 'B' && $opts->{bss}) } @binsyms;
      } else {
