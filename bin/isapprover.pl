...
use Production::Services::Move qw(isValidApprover);

#==============================================================================

=head1 NAME

isapprover.pl - Determine if users are valid approvers

=head1 SYNOPSIS

    isapprover.pl login1 login2 uuid1 uuid2 ...

=head1 DESCRIPTION

C<isapprover.pl> makes a production service query to determine whether or not
the supplied unix login names and/or UUIDs are valid approvers.
...
MAIN: {
    ...
    foreach my $user (@users) {
        my $isapprover=isValidApprover($svc,$user);

        unless (defined $isapprover) {
            error "Error occured reading approver status for $user";
	    error($svc->getError) if $svc->getError();
            exit -1;
        }

        if ($opts->{pretty}) {
            print $user." is ".($isapprover ? "" : "NOT ").
	      "a valid approver\n";
        } else {
            push @invalid,$user;
        }
