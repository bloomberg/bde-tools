...
When C<csrecover> lists files and versions it will qualify the filename with
...
sub recoverFiles {
    ...
    my $processed = $status =~ /^$STATUS_INPROGRESS|$STATUS_COMPLETE$/;
    if (-t STDIN) {
	message "change set $changeset is ".
	  ($processed ? "processed" : "staged");
    }

    $opts{released} = 1 if $processed and not $opts{staged};
    ...
    if ($opts{current}) {
        if ($opts{previous}) {
            ($ok, $error) = recoverPriorSCM($csid, $tarball, $move, 1);
        } elsif ($opts{released}) {
            ($ok, $error) = recoverCurrentSCM($csid, $tarball, $move, 0);
        } else {
            ($ok, $error) = recoverCurrentSCM($csid, $tarball, $move, 1);
        }
    } elsif ($opts{previous}) {
        ($ok, $error) = recoverPriorSCM($csid, $tarball, $move, 0);
    } else {
        ($ok, $error) = recoverFilesSCM($csid, $tarball);
    }
