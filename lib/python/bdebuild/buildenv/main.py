...
def get_compilerinfos():
    ...
    if os_type != "windows":
        ...
        return (
            user_compiler_infos
            + system_compiler_infos
            + compilerinfo.detect_installed_compilers(uplid)
        )
    else:
        compiler_infos = []
...
def print_envs(options, info):
    ...
    uplid = optiontypes.Uplid(
        os_type, os_name, cpu_type, os_ver, info.type_, info.version
    )
    ...
    if info.toolchain:
        print(
            "export BDE_CMAKE_TOOLCHAIN=toolchains/%s/%s"
            % (sysutil.unversioned_platform(), info.toolchain)
        )
    else:
        print(
            "export BDE_CMAKE_TOOLCHAIN=toolchains/%s/default"
