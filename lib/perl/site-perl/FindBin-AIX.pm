...
#### HACK HACK HACK HACK HACK HACK HACK
#### HACK HACK HACK HACK HACK HACK HACK
# This version of FindBin.pm is hacked
# to work around the ClearCase/AIX problem
# encountered in DRQS 6941641
#  -- mgiroux, 2006/05/11
   ...
   if ($^O eq 'VMS')
	...
	if(-r $scr && (!$dosish || -x _))
     ...
     unless(File::Spec->file_name_is_absolute($script)) {
         my $getcwd;
         $getcwd=getcwd() until defined $getcwd;
     ...
     if(!(defined $Bin && -e $Bin)) {
         # yield cpu, try to avoid race
         select(undef,undef,undef,rand 2);  # sleep random amount: [0,2) seconds
         goto TRY_AGAIN;
     }
    }
  }
