...
{ my %packageCache;
  ...
  sub isPackage($) {
      ...
      return ($packageCache{$_[0]}=8) if canStatPkgConfigFile($_[0]);
...
sub isComponent {
    ...
    fatal "Undefined component" unless defined $_;

    return 0 if ( $_ eq 'expat_g' || $_ eq 'sysutil_ident' );

    my $pfx = qr/(?:a_[el]|z_[ap]|z_a_[el]|z_[el]|[afmpszel])/;

    # subbordinate component
    return 5
      if /^(?:${pfx}_)?[a-z][a-z0-9]{3,}_[a-z][a-z0-9]*_[a-z][a-z0-9]*$/o;

    # application/adapter component - name must be at least 2 long, no max
    return 4
      if /^[apm]_[a-z][a-z0-9]+_[a-z][a-z0-9]*(_[a-z][a-z0-9]*)?$/;
    ...
    return 2
      if
/^(?:${pfx}_)?[a-z][a-z0-9]{2}=[a-z0-9]+_[a-z][a-z0-9]*(_[a-z][a-z0-9]*)*$/o;
...
sub isNonCompliant ($) {
    ...
    return 1 if $package_type == 3 or $package_type == 5
	     or $package_type == 6 or $package_type == 7;
    return 0;
}
...
=head2 isPkgConfig($package)

Return 1 if the supplied package name is a .pc file. Return 0 otherwise.
...
sub isPkgConfig($) {
    fatal "Undefined package" unless defined $_[0];
    my $package_type=isPackage($_[0]);
    return 1 if $package_type == IS_PKGCONFIG;
    return 0;
}
...
sub getPackageType ($) {
    ...
    return IS_PKGCONFIG if canStatPkgConfigFile($package);
...
sub isIsolatedPackage ($) {
    ...
    return 1 if $pkgtype==IS_APPLICATION or
      ...
      $pkgtype==IS_LEGACY   or $pkgtype==IS_THIRDPARTY or
      $pkgtype==IS_PKGCONFIG
...
{ my %dirs=(
    ...
    IS_PKGCONFIG()   => "." # The pkgconfig directory is not within the main repository, use "." to suppress undef-warnings
  );
  ...
  sub getFullTypeDir ($) {
      ...
      $dir = catdir($dir, INDEX_DIR) if isIndexed($type);
      return $dir;
  }
...
=head2 (set|get)FileSystemRoot($fsroot)
...

{
    ...
    sub setFileSystemRoot {
        my $new_root = shift;
        my $ref      = ref $new_root;
        die "setFileSystemRoot not passed a BDE::FileSystem but a $ref"
          unless ( UNIVERSAL::isa $ref, "BDE::FileSystem" );
        $root = $new_root;
    }
...
Return the relative path of the package, relative to the unified source tree
(/public/src) (undef if package is not found)
...
canonical location (absolute path to) unified source tree
(/public/src), but can be a
local snapshot of the package as long as the result from reading the symlink
(and removing the absolute path to the unified root) remains the same.  This
...
{   ## Legacy, Third-Party, and Untagged-Application (code enclosure block)
    ...
    sub getLegacyLeafPath ($) {
        return catdir(LEGACY_DIR, shift);
    }
...
#==============================================================================
# PkgConfig nomenclature support

=head2 canStatPkgConfigFile($package)

Return true if we can stat the pkgconfig file in one of our search paths

=cut

    sub canStatPkgConfigFile ($) {
        my $lib = shift;
...
The complete path will typically result in a target that is a symlink to the
canonical location (absolute path to) the unified source tree (/public/src),
but can be a local snapshot of the package as long as the result from reading
the symlink (and removing the absolute path to the unified root) remains the
same.  This can be done by having a symlink with a relative path to a location
within the third-party leaf of the development root (e.g.
<root>/thirdparty/...)
...
=cut
    ...
    sub getThirdPartyLeafPath ($) {
        return catdir(THIRDPARTY_DIR, shift);
    }
...
The complete path will typically result in a target that is a symlink to the
canonical location (absolute path to) the unified source tree (/public/src), but can be a
local snapshot of the package as long as the result from reading the symlink
(and removing the absolute path to the unified root) remains the same.  This
can be done by having a symlink with a relative path to a location within the
...
=cut
    ...
    sub getApplicationLeafPath ($) {
        return catdir(APPLICATIONS_DIR, shift);
    }
