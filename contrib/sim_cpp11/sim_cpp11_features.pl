...
#
#   Alternatively, by using the --no-inplace option, the output will be written
#   out to a separate file with '_cpp03' appended to the filename before the
#   extension or, by using the --output=<filename> option, to an arbitrary
#   file.
...
#endif // ! defined(COMPILING_<COMPONENT>_H)
...
#ifdef COMPILING_<COMPONENT>_CPP

EOT

my $cpp03CppCodeSuffix = <<EOT;

#endif // defined(COMPILING_<COMPONENT>_CPP)
...
#ifdef COMPILING_<COMPONENT>_T_CPP

EOT

my $cpp03TestDriverSuffix = <<EOT;

#else // if ! defined(COMPILING_<COMPONENT>_T_CPP)
...
#endif // defined(COMPILING_<COMPONENT>_T_CPP)
...
// <timestampComment>
// Command line: <commandLine>
# define COMPILING_<OUT>
# include <<cpp03>>
# undef COMPILING_<OUT>
#else
...
my @extToBoilerplateMap = (
    [ "_cpp03.t.cpp"    , $cpp03TestDriverPrefix, $cpp03TestDriverSuffix ],
    [ "_cpp03.cpp"      , $cpp03CppCodePrefix   , $cpp03CppCodeSuffix    ],
    [ "_cpp03.h"        , $cpp03HeaderPrefix    , $cpp03HeaderSuffix     ],
    [ "_cpp03"          , $cpp03HeaderPrefix    , $cpp03HeaderSuffix     ],
    [ ""                , $masterPrefix,        , $masterSuffix          ]
    );
...
#   Prologue: Segment of code before the begining of simulation directive
...
sub filenameToBoilerplate($) {
    ...
    foreach my $entry (@extToBoilerplateMap) {
        ...
        trace("filenameToBoilerplate", "ext = %s, filename = %s",
              $tail, $outputFilename);
        next unless ($tail eq "" or
                     substr($outputFilename, -length($tail)) eq $tail);
...
sub processFile($$)
{
    ...
    # end. The $preBoil string is inserted after the prologue, but only if
    # the transformation is not a no-op.
