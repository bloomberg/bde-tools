...
# Process the specified filename
sub processFile($$)
{
    ...
    while (cppSearch(qr/^[ \t]*\#[ \t]*(if(?:ndef[ \t]|[ \t]+!)[ \t]*$simCpp11Macro)\b(.*)\n/m,
                     $pos))
    {
        $variadic_limit = $variadic_limit_base . "_" . $limit++;
...
sub main() {
    ...
    usage("Must specifiy an input file name, -, or TEST") unless (@ARGV);
    usage("Only one input file name may be specified when using --output")
        unless (1 == @ARGV || ! $outputFilename);

    $dummyChar = $debug ? "." : "\267";

    my $ret = 0;
    for my $inputFilename (@ARGV) {
        usage("TEST cannot be specified with filenames")
            if ("TEST" eq $inputFilename && 1 != @ARGV);
